...
from tkinter import messagebox, ttk             # Módulos para mostrar cuadros de diálogos y widgets.
...
usuario_actual = Usuario("Nuevavida", "equipo7@mail.com")  # Usuario ficticio "logueado".
...
class AppTurnos: # Clase gestora de la ventana principal y sus interacciones.
    def __init__(self, root):  # Constructor que recibe la ventana raíz (root) y configura la interfaz.
        ...
        tk.Label(self.frame_menu, text=f"Bienvenido a  {usuario_actual.nombre}", font=("Roboto", 20)).pack(pady=10) # Mensaje de bienvenida.
        
        # Botones de menú principal.
        # command=self.metodo -> Cada botón llama a un método diferente al hacer click.
        
        tk.Button(self.frame_menu, text="Servicios Disponibles", width=25,bg= "lightpink", fg="white", command=self.mostrar_servicios).pack(pady=5) # Muestra una lista.
        tk.Button(self.frame_menu, text="Reservar Turno", width=25,bg= "lightpink", fg="white", command=self.abrir_reserva).pack(pady=5)    # Abre un formulario.
        tk.Button(self.frame_menu, text="Ver Mis Turnos", width=25,bg= "lightpink", fg="white", command=self.mostrar_turnos).pack(pady=5)   # Muestra los turnos reservados.
        tk.Button(self.frame_menu, text="Cancelar Turno", width=25,bg= "lightpink", fg="white", command=self.cancelar_turno).pack(pady=5)# Cancelación de turnos.
        tk.Button(self.frame_menu, text="Salir", width=25,bg= "lightpink", fg="white", command=root.quit).pack(pady=5)                      # Cierra la app.
        tk.Button(self.frame_menu, text="Créditos", width=15,bg= "lightpink", fg="white", command=self.mostrar_creditos).pack(pady=5)       # Boton de Credito, Integrantes Grupo 7.
    ...
    def mostrar_creditos(self):
        ...
        for persona in creditos:
            tk.Label(top, text=persona, bg="lightpink", fg="white").pack(pady=10)
creditos = [
